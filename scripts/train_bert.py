"""
Execution script for BERT Pretraining.
"""
...
from src.training.pretrain_bert import BERTMlmTrainer
...
def main():
    ...
    parser.add_argument("--config", default="configs/part2_bert_pretrain.yaml")
    ...
    # Needs a dedicated [MASK] token. SentencePiece doesn't have one by default unless added.
    # We will use UNK as MASK for simplicity, or add one. Let's use an unused ID if available, or just UNK.
    # We will use a dedicated ID (vocab_size - 1) for MASK if we assume tokenizer covers 0 to 31999.
    mask_id = tokenizer.vocab_size_actual - 1
